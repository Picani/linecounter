...
pub fn open(path: &PathBuf) -> std::io::Result<Box<dyn BufRead>> {
    if !path.exists() {
        ...
        if tree_magic::match_filepath("text/plain", &path) {
            ...
            let mut mime_type = tree_magic::from_filepath(&path);
            let mut compressed = false;

            // TODO: Clean all that...

            if tree_magic::match_filepath("application/gzip", &path) {
                compressed = true;

                let mut d = GzDecoder::new(f);
                d.read(&mut magic_buffer)?;

                if tree_magic::match_u8("text/plain", &magic_buffer) {
                    ...
                    mime_type = tree_magic::from_u8(&magic_buffer);
                }
